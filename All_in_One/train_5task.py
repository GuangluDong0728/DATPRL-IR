...
class StarIRModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        # scheduler = LinearWarmupCosineAnnealingLR(optimizer=optimizer, warmup_epochs=0, max_epochs=150) # 150/180
    ...
    def test_mode_3(self):
        ...
        if psnr_vals:
            self.log("avg_psnr_5task", avg_psnr, rank_zero_only=True)
            self.log("avg_ssim_5task", avg_ssim, rank_zero_only=True)

        self._log_epoch_results(results, avg_psnr, avg_ssim, len(psnr_vals))


    def _log_epoch_results(self, results, avg_psnr, avg_ssim, n_tasks):
        ...
        lines.append("-" * 60)
        lines.append(f"{'Avg (5-task)':<14}{avg_psnr:>10.2f}{avg_ssim:>12.4f}   "
                    f"(N={n_tasks})")
        lines.append("=" * 60)
...
def main():
    ...
    parser.add_argument('--batch_size', type=int,default=24,help="Batch size to use per GPU")
    ...
    parser.add_argument("--wblogger",type=str,default="5task",help = "Determine to log to wandb or not and the project name")
    parser.add_argument("--ckpt_dir",type=str,default="5task",help = "Name of the Directory where the checkpoint is to be saved")
    ...
    logger = TensorBoardLogger(save_dir = "5task/")
    ...
    trainer.fit(model=model, train_dataloaders=trainloader, ckpt_path="")
