...
class StarIRTrainDataset(Dataset):
    def __init__(self, args):
        super(StarIRTrainDataset, self).__init__()
        self.args = args
        self.rs_ids = []
        self.hazy_ids = []
    ...
    def _init_clean_ids(self):
        ...
        clean_ids += [os.path.join(self.args.denoise_dir, id_) for id_ in name_list
                    if any(id_.endswith(ext) for ext in extensions)]
    ...
    # 替换为
    def _init_hazy_ids(self):
        haze_dir = os.path.join(self.args.dehaze_dir, 'Haze')
        image_list = os.listdir(haze_dir)
        self.hazy_ids = [{"clean_id": os.path.join(haze_dir, x), "de_type": 4} for x in image_list]
        self.hazy_counter = 0
        self.num_hazy = len(self.hazy_ids)
        print("Total Hazy Ids : {}".format(self.num_hazy))
    ...
    # 替换为
    def _init_rs_ids(self):
        rain_dir = os.path.join(self.args.derain_dir, 'rain')
        image_list = os.listdir(rain_dir)
        self.rs_ids = [{"clean_id": os.path.join(rain_dir, x), "de_type": 3} for x in image_list]
        self.rs_ids = self.rs_ids * 120
        self.rl_counter = 0
        self.num_rl = len(self.rs_ids)
        print("Total Rainy Ids : {}".format(self.num_rl))
    ...
    # 替换为
    def _get_gt_name(self, rainy_name):
        # rain/xxx.png → norain/xxx.png
        filename = os.path.basename(rainy_name)
        gt_name = os.path.join(self.args.derain_dir, 'norain', filename)
        return gt_name
    ...
    # def _get_nonhazy_name(self, hazy_name):
    #     # Haze/xxx.png → clear/xxx.png
    #     filename = os.path.basename(hazy_name)
    #     clear_name = os.path.join(self.args.dehaze_dir, 'clear', filename)
    #     return clear_name
    def _get_nonhazy_name(self, hazy_name):
        filename = os.path.basename(hazy_name)        # 0025_0.8_0.04.jpg
        ext = '.' + filename.split('.')[-1]           # .jpg
        name = filename.split('_')[0]                 # 0025
        clear_name = os.path.join(self.args.dehaze_dir, 'clear', name + ext)
        return clear_name
    ...
    def __getitem__(self, idx):
        ...
        if de_id < 3:
            ...
            # 替换为
                ...
                degrad_img = crop_img(np.array(Image.open(os.path.join(self.args.enhance_dir, 'low', sample["clean_id"])).convert('RGB')), base=16)
                clean_img = crop_img(np.array(Image.open(os.path.join(self.args.enhance_dir, 'high', sample["clean_id"])).convert('RGB')), base=16)
                clean_name = self._get_enhance_name(sample["clean_id"])
...
class DerainDehazeDataset(Dataset):
    ...
    def _init_input_ids(self):
        if self.task_idx == 0:       # 去雨：input/ target/  ✓ 不变
            self.ids = []
            name_list = os.listdir(self.args.derain_path + 'input/')
            self.ids += [self.args.derain_path + 'input/' + id_ for id_ in name_list]

        elif self.task_idx == 1:     # 去雾：IN/ GT/  ← 修改
            self.ids = []
            name_list = os.listdir(self.args.dehaze_path + 'IN/')
            self.ids += [self.args.dehaze_path + 'IN/' + id_ for id_ in name_list]

        elif self.task_idx == 2:     # 去模糊：input/ target/  ✓ 不变
            self.ids = []
            name_list = os.listdir(self.args.gopro_path + 'input/')
            self.ids += [self.args.gopro_path + 'input/' + id_ for id_ in name_list]

        elif self.task_idx == 3:     # 增强：low/ high/  ← 修改
            self.ids = []
...
#         self.length = len(self.ids)
    ...
    def _get_gt_path(self, degraded_name):
        ...
        elif self.task_idx == 1:     # 去雾测试集：只替换 /IN/ → /GT/
            gt_name = degraded_name.replace("/IN/", "/GT/")
